...
export class Wordle {
  ...
  handleClickShare() {
    ...
    this.showInfoMessage('Risultati copiati nella clipboard');
  }
  ...
  private async checkCurrentTry() {
    ...
    if (curTry.letters.some(letter => letter.text === '')) {
      this.showInfoMessage('Non abbastanza lettere');
      return;
    }
    ...
    if (!WORDS.includes(wordFromCurTry)) {
      this.showInfoMessage('Parola inesistente');
